...
class WDSRNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, upscale=4, norm_type='weight', act_type='relu', \
        ...
        tail=[]
        skip=[]
       #fea_conv = B.conv_block(in_nc, nf//2, kernel_size=3, norm_type=None, act_type=None)
        #head.append(wn(fea_conv))
        head.append(
            wn(nn.Conv2d(in_nc, nf//2, 3, padding=3//2)))
        #中间层
        for _ in range(nb):
            body.append(B.ResNetBlock_wn(nf//2, nf*3, nf//2, norm_type=norm_type, act_type=act_type,\
            mode=mode, res_scale=res_scale))
        out_feats = upscale*upscale*out_nc
        tail.append(
                wn(B.conv_block(nf//2, out_feats, kernel_size=3, stride=1, bias=True, \
                        pad_type='zero', norm_type=norm_type, act_type=None))
                )
        tail.append(nn.PixelShuffle(upscale))
        skip.append(
                wn(B.conv_block(in_nc, out_feats, kernel_size=5, stride=1, bias=True, \
                        pad_type='zero', norm_type=norm_type, act_type=None))
                )
...
class WAResNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, upscale=4, norm_type='weight', act_type='relu', \
       ...
       #fea_conv = B.conv_block(in_nc, nf//2, kernel_size=3, norm_type=None, act_type=None)
        ...
        out_feats = upscale*upscale*out_nc
        tail.append(
                wn(B.conv_block(nf//2, 4, kernel_size=3, stride=1, bias=True, \
                        pad_type='zero', norm_type=norm_type, act_type=None))
                )
        skip.append(
                wn(B.conv_block(in_nc, 64, kernel_size=5, stride=1, bias=True, \
                        pad_type='zero', norm_type=norm_type, act_type=None))
                )
        skip.append(
                wn(B.conv_block(64, 4, kernel_size=3, stride=1, bias=True, \
                        pad_type='zero', norm_type=norm_type, act_type=None))
                )
        self.head = nn.Sequential(*head)
        self.body = nn.Sequential(*body)
...
class SRCNN(nn.Module):
    def __init__(self, in_nc, out_nc, nf, upscale=4, act_type='relu', \
        ...
        tail=[]
        head.append(
            nn.Conv2d(in_nc, nf, 9, padding=4))
        head.append(a)
        body.append(
            nn.Conv2d(nf, nf//2, 1, padding=0))
        body.append(a)
        out_feats = upscale*upscale*out_nc
        tail.append(
            nn.Conv2d(nf//2, out_feats, 5, padding=2))
        tail.append(a)
        tail.append(nn.PixelShuffle(upscale))
        self.head = nn.Sequential(*head)
        self.body = nn.Sequential(*body)
...
# A = WAResNet(5,4,64,22)
# device = torch.device('cuda:0')
# A.to(device)
# summary(A,(5,128,128),batch_size=1,device='cuda')
# A = torch.rand(1,4,128,128)
# B = torch.rand(1,1,128,128)
# C = torch.cat((A,B),1)
# D = A[:,3,:,:]
# E = D.unsqueeze(1)
# print(C.shape)
