...
FONT = ImageFont.truetype('simsun.ttc', 12, encoding='unic')
...
# def draw_label_with_scores(draw_obj, box, label, score, tower_label, color):
#     x, y = box[0], box[1]
#     draw_obj.rectangle(xy=[x, y, x + 60, y + 10],
#                        fill=color)
#     # txt = LABEl_NAME_MAP[label] + ':' + str(round(score, 2)) + str(tower_label[0]) + str(tower_label[1]) + '号杆塔'
...
# def draw_boxes_with_label_and_scores(img_array, boxes, labels, scores, tower_labels):
#
...
#     num_of_objs = 0
#     for box, a_label, a_score, tower_label in zip(boxes, labels, scores, tower_labels):
#
#         if a_label != NOT_DRAW_BOXES:
#             num_of_objs += 1
#             #draw_a_rectangel_in_img(draw_obj, box, color=STANDARD_COLORS[a_label], width=1)
...
#                  continue
#             else:
#                 draw_label_with_scores(draw_obj, box, a_label, a_score, tower_label, color='White')
#
...
if __name__ == '__main__':
    img_array = cv2.imread("F:\JWS\Backups\SRandTower\Tools\loss.png")
